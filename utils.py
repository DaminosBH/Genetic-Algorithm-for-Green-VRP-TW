...
def merge_rules(rules):
    ...
    while rules:
        current_rule = rules.pop(0)
        merged_rule = current_rule
        merged = False
        
        # Step 2: Merge overlapping rules
        for rule in rules[:]:
            if rule[0] <= current_rule[1]:
                merged_rule = (current_rule[0], max(current_rule[1], rule[1]))
                rules.remove(rule)
                merged = True
            else:
                break
        ...
        # Step 3: Check if fully merged
        if not merged:
            is_fully_merged = True
    
    return merged_rules, is_fully_merged
